...
export default function HomeworkSelectiveReadinessOverall({
  centerCode
}) {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      const pdfBlob = await html2pdf()
        ...
        .set({
          margin: 8,
          filename: "Homework_Selective_Report.pdf",
      ...
      const pdfFile = new File(
        [pdfBlob],
        `Homework_Selective_Report_${selectedStudent}.pdf`,
      ...
      const res = await fetch(
        `${BACKEND_URL}/api/admin/send-homework-selective-report-email`,
        {
          method: "POST",
          body: formData
        }
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: "Homework_Selective_Readiness_Report",
  });
  ...
  /* ============================
     Load available homework exam dates
  ============================ */
  ...
  useEffect(() => {
    ...
    fetch(
      `${BACKEND_URL}/api/admin/students/${selectedStudent}/homework-selective-report-dates`
    )
      .then(res => res.json())
      .then(data => {
  ...
  /* ============================
     Generate Overall Homework Report
  ============================ */
  ...
  const generateOverallReport = async () => {
    ...
    try {
      const res = await fetch(
        `${BACKEND_URL}/api/admin/students/${selectedStudent}/overall-selective-homework-report?exam_date=${selectedDate}`,
        { method: "POST" }
      );
  ...
  return (
    <div className="overall-readiness-container">
      <h2 className="overall-title">
        Overall Homework Selective Readiness
      </h2>
      ...
      {selectedStudent && availableDates.length > 0 && (
        <div className="selector-row">
          <label>Homework Attempt Date</label>
      ...
      {selectedStudent && selectedDate && (
        <div className="action-row">
          ...
          >
            {loading
              ? "Generating..."
              : "Generate Overall Homework Readiness Report"}
          </button>
      ...
      {error && (
        <div className="error-box">
          <h4>
            Unable to Generate Homework Report because the student has not
            attempted all four exams
          </h4>
          ...
          {error.code === "NO_EXAMS_FOUND" && (
            <p>
              The student has not attempted any homework exams on the selected
              date.
            </p>
          )}
        </div>
