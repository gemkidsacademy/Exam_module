...
export default function QuizSetup() {
  ...
  const fetchQuestionCount = async (topicName, difficulty, index) => {
  try {
    ...
    setDbCounts((prev) => ({
      ...prev,
      [`${index}_${difficulty}`]: data[difficulty] || 0   // ✅ FIX
    }));
...
const handleTopicNameChange = (index, value) => {
  ...
  // fetch for all enabled difficulties
  ["easy", "medium", "hard"].forEach((level) => {
    fetchQuestionCount(value, level, index);
  });
};
  ...
  setQuiz((prev) => {
    ...
    if (newEnabled && currentTopic.name) {
      fetchQuestionCount(currentTopic.name, level, index);
    }
    return {
      ...prev,
      topics
                    ...
                    <div>
                      <div>
                        ...
                        />
                          ...
                          <div style={{ color: "#2563eb", fontWeight: "500", marginBottom: "6px" }}>
                            Available in DB: {dbCounts[`${index}_easy`] ?? "-"}
                          </div>
                      </div>
                    </div>
                  </div>
                ...
                {topic.medium.enabled && (
                  <div className="grid-2">
                    ...
                    <div>
                      ...
                      <div style={{ color: "#2563eb", fontWeight: "500", marginBottom: "6px" }}>
                        Available in DB: {dbCounts[`${index}_medium`] ?? "-"}
                      </div>
                    </div>
                  </div>
                )}
                ...
                {topic.hard.enabled && (
                  <div className="grid-2">
                    ...
                    <div>
                      ...
                      <div style={{ color: "#2563eb", fontWeight: "500", marginBottom: "6px" }}>
                        Available in DB: {dbCounts[`${index}_hard`] ?? "-"}
                      </div>
                    </div>
                  </div>
                )}
