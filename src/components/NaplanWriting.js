  ...
  export default function NaplanWriting({
  studentId,
  mode: parentMode,
  ...
  try {
    ...
    const startExam = async () => {
      ...
      try {
        const res = await fetch(
          `${API_BASE}/api/student/start-naplan-writing-exam?student_id=${studentId}`,
          { method: "POST" }
        );
    ...
    const loadExam = async () => {
      ...
      try {
        const res = await fetch(
          `${API_BASE}/api/exams/naplan-writing/current?student_id=${studentId}`
        );
...
>
  ...
  >
    ...
    <p>
      <strong>Naplan Readiness:</strong>{" "}
    ...
    <p className="scale-note">
      Scoring is based on Naplan criteria (25 marks total).
    </p>
