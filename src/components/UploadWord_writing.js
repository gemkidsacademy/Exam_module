...
export default function UploadWord_writing() {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      alert(data.message || "Writing Word document uploaded successfully!");
      setWordFile(null);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="upload-pdf-container">
      <h2>Upload Word Document for Writing Prompts</h2>
      ...
      <p className="note">
        Upload a Word document to populate writing prompts in the database.
      </p>
    </div>
  );
}
