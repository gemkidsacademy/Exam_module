...
export default function GenerateExam_foundational() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch(`${BACKEND_URL}/api/quizzes-foundational`);
        if (!res.ok) throw new Error("Failed to load foundational quizzes");

        const data = await res.json();
        console.log("📦 Loaded foundational quizzes:", data);
        setQuizzes(data);
      } catch (err) {
  ...
  const handleGenerateExam = async () => {
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/exams/generate-foundational`, {
        method: "POST",
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        >
          ...
          {quizzes.map((q) => (
            <option
              key={q.id}
              value={JSON.stringify({ class_name: q.class_name })}
            >
              {formatClassName(q.class_name)}
            </option>
          ))}
        </select>
      </div>
