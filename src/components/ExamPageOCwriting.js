  ...
  export default function ExamPageOCWriting({
  studentId,
  mode: parentMode,
  ...
  try {
    ...
    const startExam = async () => {
      ...
      try {
        const res = await fetch(
          `${API_BASE}/api/student/start-oc-writing-exam?student_id=${studentId}`,
          { method: "POST" }
        );
    ...
    const loadExam = async () => {
      ...
      try {
        const res = await fetch(
          `${API_BASE}/api/exams/oc-writing/current?student_id=${studentId}`
        );
